...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i< batch_count; i++) {
        for(int j = 0; j< dim; j++){
            for (int k = 0; k < dim; k++)
            {
                double random_num = (double)rand() * ( 1 - 0 )  / ( double )RAND_MAX + 0;
                A[i][(dim*j) + k] = make_cuDoubleComplex(random_num,random_num);
            }
        }
    }
    ...
    clock_t begin_setup_memory = clock();
    ...
    for(int i=0; i<batch_count; i++) {
        cudaMemcpy(h_d_A[i], A[i], dim*dim*sizeof(cuDoubleComplex), cudaMemcpyHostToDevice);
        cudaMemcpy(h_d_T1[i], A[i], dim*dim*sizeof(cuDoubleComplex), cudaMemcpyHostToDevice);
    }
    clock_t end_setup_memory = clock();
...
/////////////////////////////////////////////////////////////////////////////////////////////////// SETUP END /////////////////////////////////////////////////////////////////////////////////////////////
    ...
    clock_t end_powers = clock();


    clock_t begin_norms = clock();
    // Calculate matrix norms for each powers of A for each matrix in the batch:
    for (int i = 0; i < batch_count; i++) {
        get_one_norm<<<dimGrid, dimBlock, dim*sizeof(double), streams[i%n_streams]>>>(h_d_T4[i], d_res, i, dim);
        get_one_norm<<<dimGrid, dimBlock, dim*sizeof(double), streams[i%n_streams]>>>(h_d_T6[i], d_res2, i, dim); 
        get_one_norm<<<dimGrid, dimBlock, dim*sizeof(double), streams[i%n_streams]>>>(h_d_T8[i], d_res3, i, dim); 
        get_one_norm<<<dimGrid, dimBlock, dim*sizeof(double), streams[i%n_streams]>>>(h_d_T10[i], d_res4, i, dim);
    }
    cudaDeviceSynchronize();
  ...
  /* *** Test for m_val equal to (9) ***
    ...
    */
    ...
    for (int i = 0; i < batch_count; i++) {
       ...
       //Perform scale:
        ...
        absolute_kernel<<<dimGrid_ell, dimBlock_ell, 0, streams[2]>>>(h_d_B[i], dim);
        scale_tester(handle, h_d_B[i], h_d_B[i], make_cuDoubleComplex(pow(error_coefficients[4], (1.0 / (2 * 13 + 1))), 0), dim);
        get_one_norm<<<dimGrid, dimBlock, dim*sizeof(double), streams[3]>>>(h_d_A[i], d_res4, i, dim);
        get_one_norm<<<dimGrid, dimBlock, dim*sizeof(double), streams[2]>>>(h_d_B[i], d_res3, i, dim);
    ...
    */
    ...
    clock_t begin_inverse = clock();
    Inverse_Batched(handle, d_B, d_A, dim, batch_count); 
    clock_t end_inverse = clock();
    ...
    cudaMemcpy(A[5], h_d_B[5], dim*dim*sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost);
    ...
    */
    ...
    // Operation Breakdown:
    double time_spent = (double)(end - begin) / CLOCKS_PER_SEC; // Total time
    double time_spent_setup = (double)(end_setup - begin_setup) / CLOCKS_PER_SEC; // Total time
    double time_spent_setup_memory = (double)(end_setup_memory - begin_setup_memory) / CLOCKS_PER_SEC; // Total time
    double time_spent_powers = (double)(end_powers- begin_powers) / CLOCKS_PER_SEC; // Total time
    double time_spent_norms = (double)(end_norms- begin_norms) / CLOCKS_PER_SEC; // Total time
    double time_spent_squaring = (double)(end_squaring- begin_squaring) / CLOCKS_PER_SEC; // Total time
    double time_spent_inverse = (double)(end_inverse- begin_inverse) / CLOCKS_PER_SEC; // Total time

    printf("S IS: %lf \n", s[5]);
    printf("TIME SPENT: %lf \n", time_spent);
    printf("TIME SPENT IN SETUP: %lf \n", time_spent_setup);
    printf("TIME SPENT IN SETUP MEMORY: %lf \n", time_spent_setup_memory);
    printf("TIME SPENT IN POWERS STAGE: %lf \n", time_spent_powers);
    printf("TIME SPENT IN NORMS STAGE: %lf \n", time_spent_norms);
    printf("TIME SPENT IN INVERSE STAGE: %lf \n", time_spent_inverse);
    printf("TIME SPENT IN SQUARING STAGE: %lf \n", time_spent_squaring);
