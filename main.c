...
double **get_Matrix_Powers_New(double *A, int n) {
...
void matrix_Scale_New(const double *a, double *scaled, double scale, int n) {
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
...
double ell(double *A, double *temp_new, double coeff, int m_val, int n) {
...
int main() {
    ...
    double **Tpowers = get_Matrix_Powers_New(A, n);
   ...
   // printf("%f", d6);
    ...
    if (eta1 <= theta[1] && ell(A, temp_new, error_coefficients[1], 3, n) == 0.0) {
        m_val = 3;

    }
    if (eta1 <= theta[2] && ell(A, temp_new, error_coefficients[2], 5, n) == 0.0) {
        m_val = 5;
   ...
   // printf("Norm for A8 \n");
    ...
    if (eta3 <= theta[3] && ell(A, temp_new, error_coefficients[3], 7, n) == 0.0) {
        m_val = 7;
    }

    if (eta3 <= theta[4] && ell(A, temp_new, error_coefficients[4], 0, n) == 0.0) {
        m_val = 9;
    }
    ...
    s = s + ell(A, temp_new, error_coefficients[4], 13, n);
    //printf("S IS:  |%12.5e|\n ", s);
...
//    }
    ...
    if (m_val == 13) {
        ...
        for (int k = 0; k < s; k++) { // MORE EFFICIENT WAY TO DO SQUARING --> THE SLOWEST PART
            cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, n, n, n, 1, temp_new, n, temp_new, n, 1, temp_2_new,
                        n);
