...
__global__ void why_kernel( cuDoubleComplex* A,  double* res, int k, int dim){
    extern __device__ double s[];   // Device memory array to store column sums 
    res[k] = 0;
    const int tid_x = blockDim.x*blockIdx.x + threadIdx.x;
...
__global__ void norm_1_kernel_large_alt(cuDoubleComplex* A, int dim, double* res){
   // extern __device__ double s[];   // Device memory array to store column sums 
...
double matrix_1_norm(cuDoubleComplex* d_A, cudaStream_t my_stream, int dim){
    ...
    //else{
        norm_1_kernel_large<<<dimGrid, dimBlock, dim*sizeof(double), 0>>>(d_A, dim, d_res); // Uses global memory
        printf("->HERE\n");
        //exit(0);
...
void double_pointer_swap(cuDoubleComplex*** A, cuDoubleComplex*** B){
...
//////////////// STREAMS FOR BATCH ////////////////////////
  //   cudaMemcpy(A, d_T4, dim*dim*batch_count*sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost);
  //   for (int i = 0; i < batch_count; i++) // 1-norm needs work
  // {
  //   d4[i] = pow(calculate_one_norm_New_complex(A[i],  dim), (1.0 / 4));
  // }
  //   cudaMemcpy(A, d_T6, dim*dim*batch_count*sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost);
  //   for (int i = 0; i < batch_count; i++) // 1-norm needs work
  // {
  //   d6[i] = pow(calculate_one_norm_New_complex(A[i],  dim), (1.0 / 6));
  // }
  //   cudaMemcpy(A, d_T8, dim*dim*batch_count*sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost);
  //   for (int i = 0; i < batch_count; i++) // 1-norm needs work
  // {
  //   d8[i] = pow(calculate_one_norm_New_complex(A[i],  dim), (1.0 / 8));
  // }
  //   cudaMemcpy(A, d_T10, dim*dim*batch_count*sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost);
  //   for (int i = 0; i < batch_count; i++) // 1-norm needs work
  // {
  //   d10[i] = pow(calculate_one_norm_New_complex(A[i],  dim), (1.0 / 10));
  // }
  //exit(0);
...
//cudaMemset(d_res, 0, sizeof(double)*batch_count); // Could look at multiple of these for async memory copy
for (int i = 0; i < batch_count; i++) // 1-norm needs work
{
  //norm_1_kernel_large_alt<<<dimGrid, dimBlock, dim*sizeof(double), 0>>>(h_d_T4[i], dim, d_res[0]); // Uses global memory
  why_kernel<<<dimGrid, dimBlock, dim*sizeof(double), streams[1]>>>(h_d_T4[i], d_res, i, dim); // Uses global memory
  why_kernel<<<dimGrid, dimBlock, dim*sizeof(double), streams[2]>>>(h_d_T6[i], d_res2, i, dim); // Uses global memory
  why_kernel<<<dimGrid, dimBlock, dim*sizeof(double), streams[3]>>>(h_d_T8[i], d_res3, i, dim); // Uses global memory
  why_kernel<<<dimGrid, dimBlock, dim*sizeof(double), streams[4]>>>(h_d_T10[i], d_res4, i, dim);
}
...
// Perform batch matrix multiplication
  ...
  {
    //cublasSetStream(handle, streams[i%n_streams]);
    // scale_and_add(handle, h_d_T6[i], h_d_C[i], h_d_C[i], make_cuDoubleComplex(c[i][7], 0), dim, streams[n_streams%3]);
    // scale_and_add(handle, h_d_T4[i], h_d_C[i], h_d_C[i], make_cuDoubleComplex(c[i][5], 0), dim, streams[n_streams%3]);
    // scale_and_add(handle, h_d_T2[i], h_d_C[i], h_d_C[i], make_cuDoubleComplex(c[i][3], 0), dim, streams[n_streams%3]);
    // scale_and_add(handle, h_d_A[0], h_d_C[i], h_d_C[i], make_cuDoubleComplex(c[i][1], 0), dim, streams[n_streams%3]);
  ...
  //   for (int i = 0; i < batch_count; i++)
  // {
  //   cudaMemcpy(A[i], h_d_B[i], dim*dim*sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost);
  // }
...
for (int i = 0; i < batch_count; i++){
 ...
 double_pointer_swap(&d_B, &d_C);    // Switching double pointers, need to add constraints 
}
